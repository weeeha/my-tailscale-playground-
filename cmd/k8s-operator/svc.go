...
func (a *ServiceReconciler) maybeProvision(ctx context.Context, logger *zap.SugaredLogger, svc *corev1.Service) (err error) {
	...
	// If target is 4via6 and the cluster already rejected IPv6, short-circuit
	// to avoid a wasted failing API call on every reconcile.
	if isViaTarget(sts.TailnetTargetFQDN, sts.TailnetTargetIP) {
		if cond := tsoperator.GetServiceCondition(svc, tsapi.ProxyReady); cond != nil &&
			...
			logger.Warnf("4via6 target was specified, but cluster does not support IPv6")
			return nil
		}
	}
