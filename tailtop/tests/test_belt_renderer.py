...
def test_render_hub_draws_center_card_with_self_name() -> None:
    canvas = CharCanvas(width=80, height=25)
    layout = HubLayout()
...
def test_render_bus_draws_hub_card_with_brand() -> None:
    # Hub is now on the RIGHT side; assert the branded box content appears.
...
def test_render_bus_paints_trunk_across_canvas() -> None:
    ...
    states = {
        "x": BeltState(
            ...
            out_lane=LaneState(cells_per_second=2.0),
            in_tier="busy",
            out_tier="busy",
            rx_bps=200_000.0,
            tx_bps=200_000.0,
    ...
    BeltRenderer().render_bus(
        ...
        peers_by_id=peers,
        selected_id=None,
        aggregate_rx=200_000.0,
        aggregate_tx=200_000.0,
    )
    plain = canvas.to_plain()
    # Trunk carries chevrons when traffic is non-idle
...
def test_render_bus_places_top_branch_above_trunk() -> None:
    ...
    # Peer card name "up" appears in the upper portion of the canvas
    upper = "\n".join(plain[:canvas.height // 2])
    assert "up" in upper
